...
- **閲覧(確認)モードと編集モードの切替(No.13 確定)**: 画面は既定で閲覧(確認)モードとして開き、
  現在保存されている内容を表示専用で確認できる(時刻ボタン・行の追加削除・曜日コピーはすべて
  非活性)。画面右上のボタンは閲覧モードでは「編集」と表示され、押すと編集モードへ切り替わり、
  時刻ボタンのトグル・行の追加削除・曜日間コピーが可能になる。編集モードでは同じボタンが
  「保存」に変わり、押すと BFF へ送信し、保存完了後は自動的に閲覧モードへ戻る。曜日タブの
  切り替えはどちらのモードでも可能(閲覧目的のナビゲーションのため編集操作とはみなさない)。
